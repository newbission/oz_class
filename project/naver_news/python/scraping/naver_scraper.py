...
# _yesterday = _today
_scrap_day = (
    f"{_yesterday.year}{str(_yesterday.month).zfill(2)}{str(_yesterday.day).zfill(2)}"
)

_compare_timestamp = lambda a, b: int(a) - int(b)


def _scrap_opinion(category):
    _driver.get(_basic_url + category)
    time.sleep(random.uniform(1, 4))
    while True:
        items = _driver.find_elements(By.CSS_SELECTOR, "li.opinion_serialization_item")
        last_timestamp = (
            items[-1].find_element(By.CLASS_NAME, "timestamp").text.replace(".", "")
        )
    ...
    def get_thumbnail_link(item):
        ...
        try:
            return (
                item.find_element(By.CSS_SELECTOR, ".article_item.as_type_main img")
                .get_attribute("src")
                .split("?")[0]
            )
        except:
    ...
    for item in items:
        ...
        if timestamp_diff == 0:
            temp = {
                ...
                "upload_time": None,
                "update_time": None,
                "naver_url": item.find_element(
                    By.CSS_SELECTOR, ".article_list .link"
                ).get_attribute("href"),
                "original_url": None,
                "summary": None,
...
def scrap_detail(opinions_list):
    # url을 bs4로 가져오기
    ...
    def download_img(url, title):
        ...
        try:
            ...
            file_name = f"{int(time.time())}_{hash_code}.png"
            # 파일 저장경로
            save_path = f"{current_path}/{_scrap_day}/{file_name}"
            # urllib.request.urlretrieve(url, save_path)
            with open(save_path, "wb") as file:
                file.write(response.content)
            print(title, "downaload complete")
        except:
    ...
    for opinions in opinions_list:
        for opinion in opinions:
            ...
            upload_time = get_element_attribute(
                opinion_header, "span[data-date-time]", "data-date-time"
            )
            update_time = get_element_attribute(
                opinion_header, "span[data-modify-date-time]", "data-modify-date-time"
            )
            original_url = get_element_attribute(
                opinion_header, ".media_end_head_origin_link", "href"
            )
            summary = get_opinion_summary(soup.select(".shn_text"), opinion["title"])
            thumbnail_file_name = download_img(
                opinion["thumbnail_url"], opinion["title"]
            )
            try:
                journalist = get_element(
                    opinion_header, ".media_end_head_journalist_name"
                ).text.split(" ")[0]
            except:
                journalist = None
...
def scrap():
    ...
    for category in _categories:
        opinions = _scrap_opinion(category)
        opinions_list.append(opinions)
    # 셀레니움 역할은 끝났으므로 driver 종료
    _driver.quit()

    # bs4를 활용해 기사 정보 스크랩
    scrap_detail(opinions_list)
