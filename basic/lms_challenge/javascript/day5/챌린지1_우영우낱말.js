...
function question(str) {
  ...
  for(const c of str.toUpperCase()){
    if(c.charCodeAt() < 'A'.charCodeAt() || c.charCodeAt() > 'Z'.charCodeAt()) continue;
    temp += c;
  }
  const half = Math.floor(temp.length / 2);
  const a = temp.slice(0, temp.length % 2 ? half+1 : half);
  const b = temp.slice(half).split('').reverse().join('');
